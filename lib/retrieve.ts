...
export async function retrieve(
  ...
  const { rows } = await getPool().query(
    `SELECT
     ...
     WHERE d.session_id = $4                     -- only this browser session's own documents
       AND 1 - (c.embedding <=> $1) >= $3         -- similarity floor: drop irrelevant hits
     ORDER BY c.embedding <=> $1                  -- ascending distance = descending similarity;
                                                   -- lets the HNSW index accelerate the search
     LIMIT $2`,
    [toVec(qVec), k, minSimilarity, sessionId]
  );
